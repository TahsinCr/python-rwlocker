...
* **Безопасность при отмене (Cancellation Safety):** Полная устойчивость к отменам задач (`CancelledError`) в среде `asyncio`. Отмененные задачи не повреждают состояние системы и безопасно пробуждают ожидающие задачи.
* **Очередь Condition за O(1) (Stampede Protection):** В отличие от стандартных библиотек, не выполняет сканирование за O(N) при вызове `notify_all()`. Мгновенно пробуждает сотни задач, не перегружая CPU.
* **Умная сигнализация (Smart Signaling):** Способность точно пробудить только необходимое количество задач, например `notify(n=5)`, не создавая эффекта «Thundering Herd» в системе.
* **Безупречная защита от отмены (Cancellation Shielding):** Если задача отменяется извне (`CancelledError`) во время ожидания в асинхронном `Condition.wait()`, состояние блокировки никогда не повреждается. Блокировка безопасно перехватывается и передается другим ожидающим.
* **100% Drop-in замена (Drop-in Replacement):** Вы можете внедрять свои продвинутые блокировки (`RWLockFair` и др.) напрямую в сторонние библиотеки (SQLAlchemy, requests, FastAPI и др.), ожидающие стандартные экземпляры `threading.Lock` или `asyncio.Lock`, не внося никаких изменений в код. Стандартные вызовы API (например, `lock.acquire()`) автоматически и безопасно перенаправляются на эксклюзивный прокси `.write`.
* **Изоляция производительности "Happy Path":** Микроочередная архитектура нового поколения, устойчивая к прерываниям ОС, которая полностью обходит дорогостоящие операции очистки O(N) при успешных пробуждениях блокировки, завершая процесс с нулевыми затратами процессора.
* **Стандартные адаптеры (Standard Adapters):** Классы-обертки `Lock`, `Condition`, `AsyncLock` и `AsyncCondition` для внедрения зависимостей (DI), когда сложные возможности RWLock не требуются, но желательна та же архитектурная сигнатура (`.read`, `.write`).
...
| **Справедливая (Fair)** | `RWLockFair` / `AsyncRWLockFair` | Предоставляет доступ поочередно читателям и писателям. Предотвращает голодание обеих сторон. | При высокочастотном двунаправленном трафике (MAVLink, WebSockets и др.). |
...
**🧪 Методология тестирования:** Чтобы обеспечить абсолютную точность и нулевую погрешность, все сущности читателей и писателей (потоки или асинхронные задачи) создаются заранее и удерживаются на стартовой линии с помощью барьера синхронизации `Event` (Событие). Как только событие срабатывает, они начинают выполнение одновременно. Рабочие нагрузки строго следуют сценарию `IOBoundScenario`, который применяет точную задержку `time.sleep(0.001)` или `asyncio.sleep(0.001)` для точного моделирования реальной задержки ввода-вывода сети/БД.
...
Когда 100 спящих читателей пробуждаются одновременно, `rwlocker` обрабатывает их мгновенно, не блокируя ОС. Этот архитектурный скачок приводит к умопомрачительному **ускорению в ~44 раза** по сравнению с `threading.Condition` из стандартной библиотеки.
<p align="center">
...
В системах кэширования, управляемых событиями, одновременное пробуждение сотен ожидающих веб-запросов является серьезным узким местом. `AsyncRWCondition` полностью обходит стандартные ограничения asyncio, достигая **пропускной способности в ~42 раза выше** во время сценариев массовой трансляции, полностью спасая цикл событий от зависания.
<p align="center">
  <img src="./figures/async_rwcondition.svg" alt="Async RWCondition Benchmark" width="100%"/>
</p>
<br>

*(Примечание: Все классы блокировок, адаптеров и условий прошли массивный набор из **315 различных модульных тестов (unit tests)**, охватывающих реентерабельность, взаимные блокировки, таймауты, прерывания ОС, утечки O(N) и сценарии безопасности при отмене с 0 ошибок, и весь этот набор тестов был завершен всего за **13.2 секунды**.)*
